...
struct nilai
{
	string npm, kode, mataKuliah, semester, huruf,
		   tugas, quiz, uts, uas, akhir;
} Nilai[100];

HWND username, password;
HWND npm, nama, program;
HWND kode, mataKuliah, semester, huruf,
	 tugas, quiz, uts, uas;
...
//PENGATURAN PRINTER
//char	*cetakJudul = "DATA PENILAIAN",
//		*cetakPrinter = "NAMA PRINTER";
int		cetakFont = 14;
...
void _menu(HWND hw)
{
	...
	AppendMenu(menu, MF_POPUP, PRINT, "PRINT");
	...
	if(statusLogin == false)
	{
		...
		EnableMenuItem(menu, PRINT, MF_GRAYED);
	}
	else
	{
...
//	static HWND dataNilai;
	bersihkan(hw);
	CreateWindow("button", "Edit", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON | SS_CENTER,230,10,100,24,hw, NULL, NULL, NULL);
	CreateWindow("button", "Delete", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON | SS_CENTER,331,10,100,24,hw, NULL, NULL, NULL);
	...
	for(auto& n:vecNilai)
	{
		for(auto& mhs:vecMahasiswa)
		{
			if(strcmp(n.npm.c_str(), mhs.npm.c_str()) == 0)
			{
				...
				ListView_SetItemText(dataNilai, no,1, const_cast<LPSTR>(n.npm.c_str()));
				ListView_SetItemText(dataNilai, no,2, const_cast<LPSTR>(mhs.nama.c_str()));
				ListView_SetItemText(dataNilai, no,3, const_cast<LPSTR>(mhs.program.c_str()));
				ListView_SetItemText(dataNilai, no,4, const_cast<LPSTR>(n.kode.c_str()));
				ListView_SetItemText(dataNilai, no,5, const_cast<LPSTR>(n.mataKuliah.c_str())); 
				ListView_SetItemText(dataNilai, no,6, const_cast<LPSTR>(n.semester.c_str()));
				ListView_SetItemText(dataNilai, no,7, const_cast<LPSTR>(n.tugas.c_str()));
				ListView_SetItemText(dataNilai, no,8, const_cast<LPSTR>(n.quiz.c_str()));
				ListView_SetItemText(dataNilai, no,9, const_cast<LPSTR>(n.uts.c_str()));
				ListView_SetItemText(dataNilai, no,10, const_cast<LPSTR>(n.uas.c_str()));
				ListView_SetItemText(dataNilai, no,11, const_cast<LPSTR>(n.akhir.c_str()));
				ListView_SetItemText(dataNilai, no,12, const_cast<LPSTR>(n.huruf.c_str()));
				no++;
			}
		}
	}
